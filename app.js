...
async function estimateDrinkingWindow(){
  ...
  if(!name||!vintage){toast("Confirm the wine name and vintage before estimating a drinking window.");return}
  const payload={
    name,
  ...
  try{
    ...
    box.innerHTML=`<h4>Estimated drinking guidance</h4>${confidenceBadge(data.confidence==="insufficient"?"low":data.confidence)}
      ...
      <p class="enrichment-estimate"><strong>AI estimate:</strong> ${esc(data.warning||"Bottle condition, storage history and personal taste can change the ideal drinking window.")}</p>
    ...
    smartMeta={...smartMeta,enrichmentMethod:"gemini-ai-estimate",enrichmentConfidence:data.confidence||"low",enrichmentAt:new Date().toISOString(),peakFromYear:data.peakFromYear||null,peakToYear:data.peakToYear||null,foodPairings:pairs,servingTemperature:data.servingTemperature||"",styleSummary:data.styleSummary||""};
